...
def batchbangun(user, candi, bahan):
    ...
    for i in range (user[1][0]):
        if user[0][i][2] == "jin_pembangun":
            count += 1
            pembangun = myappend(pembangun, [user[0][i][0]])
    if count == 0:
        ...
        if not pasirada and not batuada and not airada:
            ...
            for i in range (count):
                ...
                stone = int(rng(1))
                water = int(rng(1))
                canditemp = myappend(canditemp, [candi[1][0], pembangun[0][i+1][0], sand, stone, water])
                bahantemp[0] += sand
                bahantemp[1] += stone
                bahantemp[2] += water
                jumlah += 1
            if bahantemp[0] > int(bahan[0][idxpasir][2]) or bahantemp[1] > int(bahan[0][idxbatu][2]) or bahantemp[2] > int(bahan[0][idxair][2]):
                print(f"Bangun gagal. Kurang {bahantemp[0] - bahan[0][idxpasir][2]} pasir, {bahantemp[1] - bahan[0][idxbatu][2]} batu, dan {bahantemp[2] - bahan[0][idxair][2]} air.")
            else:
